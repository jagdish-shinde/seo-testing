...
export default function ProfileCardV1({alumni = []}) {
  ...
  return (
    <main className={styles.profileCardWrapper}>
        <picture className={styles.imageContainer}>
            <Image src={profile} height={"100%"} width={"100%"} layout='responsive' objectFit="cover" />
        </picture>
        <div className={styles.infoContainer}>
                <p className={styles.userName}>{name}</p>
                <p className={styles.careerDetails}>{designation && `${designation} @ `}{currentCompany}</p>
                <p className={styles.batchDetails}>{`Batch ${graduationYear}`}</p>
